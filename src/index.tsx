...
import { getDaysOfCalendarMonth, findEventsForTheDay, CONTAINER_WIDTH } from './utils'
...
class MonthViewCalendar extends React.Component<MonthViewProps, MonthViewState> {
    ...
    scrollEnd = (event: NativeSyntheticEvent<NativeScrollEvent>) => {
        ...
        } else if (movedPages > 0 && newIndex >= this.state.datesList.length - this.pageOffset) {
                    ...
                    renderItem={({ item }) => {
                        return (
                            <View style={{ width: CONTAINER_WIDTH }}>
                                <CalendarHeader weekDays={weekDays} textStyles={headerTextStyles} />
                                <View style={styles.monthViewExceptHeader}>{this.renderWeekCalendar(item)}</View>
                            </View>
                        )
                    }}
                    pagingEnabled
